...
class EV_Central:
    def __init__(self):
        self.tools = AppArgumentParser("EV_Central", "Sistema Central de Control para Puntos de Recarga de Vehículos Eléctricos")
        ...
        self.tools.add_argument("--db", type=ip_port_type, help="IP y puerto del servidor de base de datos (formato IP:PORT)", default=("localhost", 5432))
        self.args = self.tools.parse_args()
    
    def start(self):
        print(f"Starting EV Central on port {self.args.listen_port}")
        print(f"Connecting to Broker at {self.args.broken[0]}:{self.args.broken[1]}")
